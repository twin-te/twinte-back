...
var TagWhere = struct {
	...
	UserID:   whereHelperstring{field: "\"tags\".\"user_id\""},
	Name:     whereHelperstring{field: "\"tags\".\"name\""},
	Position: whereHelperint{field: "\"tags\".\"position\""},
}
...
type tagR struct {
	RegisteredCourses RegisteredCourseSlice `boil:"RegisteredCourses" json:"RegisteredCourses" toml:"RegisteredCourses" yaml:"RegisteredCourses"`
}
...
func (r *tagR) GetRegisteredCourses() RegisteredCourseSlice {
	if r == nil {
		return nil
	}
	return r.RegisteredCourses
}
...
var (
	tagAllColumns            = []string{"id", "user_id", "name", "position"}
	tagColumnsWithoutDefault = []string{"id", "user_id", "name", "position"}
...
func (q tagQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Tag, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dbmodel: failed to execute a one query for tags")
	}
...
func (q tagQuery) All(ctx context.Context, exec boil.ContextExecutor) (TagSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "dbmodel: failed to assign all query results to Tag slice")
	}
...
func (q tagQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: failed to count tags rows")
	}
...
func (q tagQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "dbmodel: failed to check if tags exists")
	}

	return count > 0, nil
}

// RegisteredCourses retrieves all the registered_course's RegisteredCourses with an executor.
func (o *Tag) RegisteredCourses(mods ...qm.QueryMod) registeredCourseQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"registered_course_tags\" on \"registered_courses\".\"id\" = \"registered_course_tags\".\"registered_course\""),
		qm.Where("\"registered_course_tags\".\"tag\"=?", o.ID),
	)

	return RegisteredCourses(queryMods...)
}

// LoadRegisteredCourses allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (tagL) LoadRegisteredCourses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	var slice []*Tag
	var object *Tag
	...
	query := NewQuery(
		qm.Select("\"registered_courses\".\"id\", \"registered_courses\".\"user_id\", \"registered_courses\".\"year\", \"registered_courses\".\"course_id\", \"registered_courses\".\"name\", \"registered_courses\".\"instractor\", \"registered_courses\".\"credit\", \"registered_courses\".\"methods\", \"registered_courses\".\"schedules\", \"registered_courses\".\"memo\", \"registered_courses\".\"attendance\", \"registered_courses\".\"absence\", \"registered_courses\".\"late\", \"a\".\"tag\""),
		qm.From("\"registered_courses\""),
		qm.InnerJoin("\"registered_course_tags\" as \"a\" on \"registered_courses\".\"id\" = \"a\".\"registered_course\""),
		qm.WhereIn("\"a\".\"tag\" in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load registered_courses")
	}

	var resultSlice []*RegisteredCourse

	var localJoinCols []string
	for results.Next() {
		one := new(RegisteredCourse)
		var localJoinCol string

		err = results.Scan(&one.ID, &one.UserID, &one.Year, &one.CourseID, &one.Name, &one.Instractor, &one.Credit, &one.Methods, &one.Schedules, &one.Memo, &one.Attendance, &one.Absence, &one.Late, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for registered_courses")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice registered_courses")
		}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on registered_courses")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for registered_courses")
	}
	...
	if singular {
		object.R.RegisteredCourses = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &registeredCourseR{}
			}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.RegisteredCourses = append(local.R.RegisteredCourses, foreign)
				if foreign.R == nil {
					foreign.R = &registeredCourseR{}
				}
...
// AddRegisteredCourses adds the given related objects to the existing relationships
// of the tag, optionally inserting them as new records.
// Appends related to o.R.RegisteredCourses.
// Sets related.R.Tags appropriately.
func (o *Tag) AddRegisteredCourses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RegisteredCourse) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"registered_course_tags\" (\"tag\", \"registered_course\") values ($1, $2)"
	...
	if o.R == nil {
		o.R = &tagR{
			RegisteredCourses: related,
		}
	} else {
		o.R.RegisteredCourses = append(o.R.RegisteredCourses, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &registeredCourseR{
				Tags: TagSlice{o},
			}
		} else {
...
// SetRegisteredCourses removes all previously related items of the
// tag replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Tags's RegisteredCourses accordingly.
// Replaces o.R.RegisteredCourses with related.
// Sets related.R.Tags's RegisteredCourses accordingly.
func (o *Tag) SetRegisteredCourses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RegisteredCourse) error {
	query := "delete from \"registered_course_tags\" where \"tag\" = $1"
	...
	removeRegisteredCoursesFromTagsSlice(o, related)
	if o.R != nil {
		o.R.RegisteredCourses = nil
	}

	return o.AddRegisteredCourses(ctx, exec, insert, related...)
}

// RemoveRegisteredCourses relationships from objects passed in.
// Removes related items from R.RegisteredCourses (uses pointer comparison, removal does not keep order)
// Sets related.R.Tags.
func (o *Tag) RemoveRegisteredCourses(ctx context.Context, exec boil.ContextExecutor, related ...*RegisteredCourse) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"registered_course_tags\" where \"tag\" = $1 and \"registered_course\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.RegisteredCourses {
			if rel != ri {
				continue
			}

			ln := len(o.R.RegisteredCourses)
			if ln > 1 && i < ln-1 {
				o.R.RegisteredCourses[i] = o.R.RegisteredCourses[ln-1]
			}
			o.R.RegisteredCourses = o.R.RegisteredCourses[:ln-1]
			break
		}
	}
...
func removeRegisteredCoursesFromTagsSlice(o *Tag, related []*RegisteredCourse) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func FindTag(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Tag, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dbmodel: unable to select from tags")
	}
...
func (o *Tag) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("dbmodel: no tags provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodel: unable to insert into tags")
	}
...
func (o *Tag) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("dbmodel: unable to update tags, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to update tags row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: failed to get rows affected by update for tags")
	}
...
func (q tagQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to update all for tags")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to retrieve rows affected for tags")
	}
...
func (o TagSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to update all in tag slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to retrieve rows affected all in update all tag")
	}
	return rowsAff, nil
}
...
func (o *Tag) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("dbmodel: no tags provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("dbmodel: unable to upsert tags, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodel: unable to upsert tags")
	}
...
func (o *Tag) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("dbmodel: no Tag provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to delete from tags")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: failed to get rows affected by delete for tags")
	}
...
func (q tagQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("dbmodel: no tagQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to delete all from tags")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: failed to get rows affected by deleteall for tags")
	}
...
func (o TagSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to delete all from tag slice")
	}
...
func (o *TagSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "dbmodel: unable to reload all in TagSlice")
	}
...
func TagExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "dbmodel: unable to check if tags exists")
	}
